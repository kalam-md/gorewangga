...
<main class="flex-1 overflow-y-auto p-4">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Tambah Lapangan Baru</h2>
                <p class="text-gray-600 mt-1">Lengkapi informasi lapangan olahraga</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="{{ route('lapangan.store') }}" method="POST" enctype="multipart/form-data" id="lapanganForm">
            @csrf
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <select id="jenis" 
                        ...
                        <option value="futsal" {{ old('jenis') == 'futsal' ? 'selected' : '' }}>
                            <i class="fas fa-futbol"></i> Futsal
                        </option>
                        <option value="basket" {{ old('jenis') == 'basket' ? 'selected' : '' }}>
                            <i class="fas fa-basketball-ball"></i> Basket
                        </option>
                        <option value="badminton" {{ old('jenis') == 'badminton' ? 'selected' : '' }}>
                            <i class="fas fa-table-tennis"></i> Badminton
                        </option>
                        <option value="tenis" {{ old('jenis') == 'tenis' ? 'selected' : '' }}>
                            <i class="fas fa-tennis-ball"></i> Tenis
                        </option>
                    </select>
                    @error('jenis')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" 
                              ...
                              placeholder="Deskripsi detail tentang lapangan, fasilitas yang tersedia, dll.">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="border-2 border-gray-300 border-dashed rounded-lg p-6">
                        <div class="text-center">
                            ...
                            <p class="text-xs text-gray-500">PNG, JPG, GIF sampai 2MB (maksimal 5 file)</p>
                        </div>
                    </div>
                    @error('gambar.*')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
